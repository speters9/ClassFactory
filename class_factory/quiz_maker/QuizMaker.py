...
class QuizMaker(BaseModel):
        ...
        for lesson_idx, (lesson_no, readings) in enumerate(self.readings.items(), 1):
            ...
            for reading_idx, reading in enumerate(readings, 1):
                ...
                while not valid and retries < MAX_RETRIES:
                    ...
                    # If response is a pydantic object, use .model_dump(), else parse as JSON
                    if hasattr(response, 'model_dump'):
                        quiz_questions = response.model_dump()
                    else:
                    ...
                    val_response = self._validate_llm_response(quiz_questions=quiz_questions,
                                                               ...
                                                               difficulty_level=difficulty_level,
                                                               additional_guidance=additional_guidance,
                                                               task_schema=Quiz.model_json_schema())
    ...
    def _parse_llm_questions(self, quiz_questions: Union[Dict, List, Any]) -> List[Dict]:
        """
        Process LLM-generated questions for consistent structure and formatting.

        Args:
            quiz_questions (Union[Dict, List, Any]): Questions generated by the LLM, structured as a pydantic object, dictionary by type, or a list.
        ...
        if hasattr(quiz_questions, 'model_dump'):
            quiz_questions = quiz_questions.model_dump()
        # If it's a dict, iterate by type
        if isinstance(quiz_questions, dict):
            for question_type, questions in quiz_questions.items():
                for question in questions:
                    # If question is a pydantic object, convert to dict
    ...
    def _validate_llm_response(self, quiz_questions: Dict[str, Any], objectives: str, reading: str,
                               prior_quiz_questions: List[str], difficulty_level: int, additional_guidance: str, task_schema: str = Quiz.model_json_schema()) -> Dict[str, Any]:
        """
        ...
        for question_num, question in enumerate(questions):
            try:
                ...
                if correct_answer not in ['A', 'B', 'C', 'D']:
                    ...
                    if not matched:
                        self.logger.warning(
                            f"Could not match correct_answer '{correct_answer}' to any option in question {
                                question_num}. Adding to rejected questions."
                        )
    ...
    def _build_quiz_chain(self):
        ...
        """
        # Create the chain with the prompt, LLM, and structured output
        chain = self.quiz_prompt | self.llm.with_structured_output(Quiz)
    ...
    def save_quiz(self, quiz: List[Dict]) -> None:
        ...
        Args:
            quiz (List[Dict[str, Any]]): List of quiz questions to save. Each dict should contain:
                ...
                - A/B/C/D or A)/B)/C)/D) (str): Answer choices
                - correct_answer (str): Letter of correct answer

        Returns:
            None
        """
        # Standardize answer keys to 'A)', 'B)', 'C)', 'D)'
        standardized_quiz = []
        for q in quiz:
            q_std = q.copy()
            for opt in ['A', 'B', 'C', 'D']:
                if opt in q_std and f'{opt})' not in q_std:
                    q_std[f'{opt})'] = q_std[opt]
                # Remove non-standard keys if both exist
                if opt in q_std and f'{opt})' in q_std:
                    del q_std[opt]
                # Ensure all options exist
                if f'{opt})' not in q_std:
                    q_std[f'{opt})'] = ''
            # Ensure all required keys exist
            for key in ['type', 'question', 'correct_answer']:
                if key not in q_std:
                    q_std[key] = ''
            standardized_quiz.append(q_std)

        final_quiz = pd.DataFrame(standardized_quiz)
        col_order = ['type', 'question', 'A)', 'B)', 'C)', 'D)', 'correct_answer']
        for col in col_order:
            if col not in final_quiz:
                final_quiz[col] = ''
...
if __name__ == "__main__":
    ...
    slideDir = user_home / class_config['slideDir']
